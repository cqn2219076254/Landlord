...
public class CP1CardDeal : MonoBehaviour
{
    ...
    IEnumerator CreateCard()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            Game2Card p = g.GetComponent<Game2CardUI>().card = new Game2Card(mycord[7 - i] % 13, mycord[7 - i] / 13, "", g);
            p.host = "player";
            p.sortOrder = i;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            g.GetComponent<SpriteRenderer>().sortingOrder = i;
            g.transform.localScale = scale;
            Game2Card p = g.GetComponent<Game2CardUI>().card =
                new Game2Card(cpcord[3 - i] % 13, cpcord[3 - i] / 13, "", g);
